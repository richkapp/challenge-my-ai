...
import type { MemoryPairingTransactionCoordinator } from "@/lib/agent-pairing/storage";
import { canonicalizeNewChallengeBrief, challengeCriteriaHistory, evaluatePersistedChallengeClosure, initializeChallengeCriteriaPersistence, refreshChallengePublicEligibility, revisePersistedChallengeCriteria } from "@/db/migrations/challenge-criteria-v1";
...
export function getAgentFeedStoreReadiness(): AgentFeedStoreReadiness {
  return inspectAgentFeedState(loadState() as unknown as Record<string, unknown>);
}

function agentFeedMutate<T>(operation: (state: State) => T): T {
  const working = clone(loadState());
  const readiness = inspectAgentFeedState(working as unknown as Record<string, unknown>);
  if (!readiness.ready) throw new Error(`Agent feed store is not ready: ${readiness.reason}.`);
  const result = operation(working);
  assertAgentFeedPersistedRootV1(working as unknown as Record<string, unknown>);
  saveState(working);
  return result;
}

export function createLocalAgentProtocolTransactionCoordinator(): MemoryPairingTransactionCoordinator {
  return {
    begin() {
      const working = clone(loadState());
      const readiness = inspectAgentFeedState(working as unknown as Record<string, unknown>);
      if (!readiness.ready) throw new Error(`Agent feed store is not ready: ${readiness.reason}.`);
      let committed = false;
      const agentFeedStore: AgentFeedTransactionalStore = {
        transactAgentFeedRequest(input, execute, transactionTime = new Date(now())) {
          return transactAgentFeedRequestState(working, input, execute, transactionTime);
        },
        submitAgentFeedContribution(input, transactionTime = new Date(now())) {
          return submitAgentFeedContributionState(working, input, transactionTime);
        },
      };
      return {
        agentFeedStore,
        assertCoherence(pairingState: PairingPlatformState) {
          assertAgentProtocolStateCoherence(pairingState, working as unknown as Record<string, unknown>);
        },
        commit() {
          if (committed) throw new Error("Local Agent protocol transaction was already committed.");
          assertAgentFeedPersistedRootV1(working as unknown as Record<string, unknown>);
          saveState(working);
          committed = true;
        },
...
export function queryAgentFeed(input: AgentFeedQueryInput): AgentFeedQueryResult {
  const state = clone(loadState());
  const readiness = inspectAgentFeedState(state as unknown as Record<string, unknown>);
  if (!readiness.ready) throw new Error(`Agent feed store is not ready: ${readiness.reason}.`);
  return queryAgentFeedState(state, input);
}

export function transactAgentFeedRequest(
  input: AgentFeedRequestTransactionInput,
  execute: AgentFeedRequestExecutor,
  transactionTime = new Date(now()),
): AgentFeedRequestTransactionResult {
  return agentFeedMutate((state) => transactAgentFeedRequestState(state, input, execute, transactionTime));
}

export function submitAgentFeedContribution(
  input: AgentFeedSubmissionInput,
  transactionTime = new Date(now()),
): AgentSubmissionAcceptResult {
  return agentFeedMutate((state) => submitAgentFeedContributionState(state, input, transactionTime));
}

export function withAgentFeedTransaction<T>(operation: (transaction: AgentFeedTransaction) => T, transactionTime = new Date(now())): T {
  return agentFeedMutate((state) => operation(createAgentFeedTransaction(state, transactionTime)));
}
...
export function createChallenge(input: { id?: string; posterId?: string; visibility: "public" | "private"; reward: number; brief: ChallengeBrief; safetyFlags?: SafetyFlag[] }): Challenge {
  return mutate((state) => {
    const at = now();
...
export function listChallenges(filters: Partial<{ category: string; mode: ContributionMode; status: string }> = {}): Challenge[] {
  const state = loadState();
...
export function getChallengeCriteriaHistory(challengeId: string) {
  const state = loadState();
...
export function updateChallengeCriteria(input: {
  challengeId: string;
  posterId: string;
...
export function evaluateChallengeClosure(input: {
  challengeId: string;
  posterId: string;
...
export function createContribution(input: { challengeId: string; contributorId?: string; contributorKind?: "human" | "agent"; contributorLabel?: string; card: ContributionCard; externallyGenerated?: boolean }): Contribution {
  return mutate((state) => withDerivedContribution(state, createContributionInState(state, input)));
}

export function createAgentContribution(input: { agentId: string; agentLabel?: string; ownerId?: string; challengeId: string; card: ContributionCard; externallyGenerated?: boolean }): Contribution {
  return mutate((state) => {
...
export function listContributions(challengeId: string): Contribution[] {
  const state = loadState();
...
export function rateContribution(input: { contributionId: string; raterId?: string; usefulness: number; novelty?: number; correctness?: number; safety?: number; comment?: string }): Rating {
  return mutate((state) => {
...
export function communityVote(contributionId: string, value: CommunityVoteValue, voterId = "local-community"): CommunityVoteResult {
  return mutate((state) => {
...
export function appendCredit(input: { userId: string; challengeId?: string; contributionId?: string; amount: number; reason: string; kind?: CreditEvent["kind"]; source?: CreditEvent["source"]; idempotencyKey?: string; metadata?: CreditEvent["metadata"] }): CreditEvent {
  return mutate((state) => appendCreditEventInState(state, input));
}

export function listCreditEvents(userId?: string): CreditEvent[] {
  return loadState().creditEvents.filter((event) => !userId || event.userId === userId).map(clone);
}

export function getPublicContributorProfileData(contributorId: string) {
  return selectPublicContributorProfileData(loadState(), contributorId);
}

export function createJob(input: { challengeId?: string; kind: Job["kind"]; provider?: string; model?: string; promptVersion?: string }): Job {
  return mutate((state) => {
    const at = now();
...
export function getJob(id: string): Job | undefined {
  const job = loadState().jobs.find((item) => item.id === id);
  return job ? clone(job) : undefined;
}

export function synthesizeChallenge(challengeId: string): SynthesisBrief {
  return mutate((state) => {
    const challenge = state.challenges.find((item) => item.id === challengeId);
    if (!challenge) throw new Error("Challenge not found.");
...
export function getLatestSynthesis(challengeId: string): SynthesisBrief | undefined {
  const brief = loadState().synthesisBriefs.find((item) => item.challengeId === challengeId);
  return brief ? clone(brief) : undefined;
}

export function reportTarget(input: { targetType: ModerationTargetType; targetId: string; reason: string; actorId?: string; note?: string }): ModerationEvent {
  return mutate((state) => {
    const target = resolveModerationTargetInState(state, input);
...
export function listModerationEvents(limit = 100): ModerationEvent[] {
  return loadState().moderationEvents.slice(0, limit).map(clone);
}

export function moderateTarget(input: { targetType: ModerationTargetType; targetId: string; action: Exclude<ModerationAction, "report">; reason: string; actorId?: string; note?: string }): ModerationActionResult {
  return mutate((state) => moderateTargetInState(state, input));
}

export function suppressChallenge(id: string, reason: string, actorId = "local-moderator", note?: string): ModerationActionResult {
  return moderateTarget({ targetType: "challenge", targetId: id, action: "suppress", reason, actorId, note });
}

export function restoreChallenge(id: string, reason: string, actorId = "local-moderator", note?: string): ModerationActionResult {
  return moderateTarget({ targetType: "challenge", targetId: id, action: "restore", reason, actorId, note });
}

export function suppressContribution(id: string, reason: string, actorId = "local-moderator", note?: string): ModerationActionResult {
  return moderateTarget({ targetType: "contribution", targetId: id, action: "suppress", reason, actorId, note });
}

export function restoreContribution(id: string, reason: string, actorId = "local-moderator", note?: string): ModerationActionResult {
  return moderateTarget({ targetType: "contribution", targetId: id, action: "restore", reason, actorId, note });
}

export function resolveAgent(input: { id: string; label?: string; ownerId?: string; description?: string; capabilities?: ContributionMode[] }): AgentProfile {
  return mutate((state) => withDerivedAgent(state, ensureAgentInState(state, input)));
}

export function ensureDemoAgent(): AgentProfile {
  return mutate((state) => withDerivedAgent(state, ensureAgentInState(state, demoAgentConfig, { touch: false })));
}
...
export function listAgentActivity(limit = 30): AgentActivity[] {
  return loadState().agentActivity.slice(0, limit).map(clone);
}

export function recordAgentActivity(input: { agentId: string; agentLabel?: string; ownerId?: string; action: AgentActivityAction; summary: string; challengeId?: string; contributionId?: string }): AgentActivity {
  return mutate((state) => {
...
export function watchChallenge(input: { agentId: string; agentLabel?: string; ownerId?: string; challengeId: string }): { agent: AgentProfile; watch: AgentWatch; activity: AgentActivity } {
  return mutate((state) => {
    const challenge = state.challenges.find((item) => item.id === input.challengeId);
...
export function resolveAgentHome(input: { ownerId: string; ownerLabel?: string }): AgentHome {
  return mutate((state) => withDerivedAgentHome(state, ensureAgentHomeInState(state, input)));
}

export function createAgentHomeConnection(input: { ownerId: string; ownerLabel?: string; displayLabel?: string; provider?: string; defaultModel?: string; allowedModels?: string[]; allowedRequestClasses?: ContributionMode[]; providerSecret?: string }): { agentHome: AgentHome; connection: AgentConnection } {
  return mutate((state) => {
...
export function updateAgentHomeConnection(input: { ownerId: string; connectionId: string; action: "pause" | "resume" | "rotate" | "reconnect" | "revoke"; providerSecret?: string }): { agentHome: AgentHome; connection: AgentConnection } {
  return mutate((state) => {
...
export function getAgentConnectionCredential(input: { ownerId: string; connectionId: string }) {
  const state = loadState();
  const connection = state.agentConnections.find((item) => item.id === input.connectionId && item.ownerId === input.ownerId);
...
export function replaceAgentConnectionCredential(input: { ownerId: string; connectionId: string; expectedRevision: number; value: unknown }): { updated: boolean; credential?: AgentCredentialRecord; connection?: AgentConnection } {
  return mutate((state) => {
    const connection = state.agentConnections.find((item) => item.id === input.connectionId && item.ownerId === input.ownerId);
...
export function markAgentConnectionNeedsReconnect(input: { ownerId: string; connectionId: string; reason: string }): AgentConnection | undefined {
  return mutate((state) => {
    const connection = state.agentConnections.find((item) => item.id === input.connectionId && item.ownerId === input.ownerId);
    if (!connection) return undefined;
    const at = now();
...
export function getRuntimeSecret(input: { ref: string }): string | undefined {
  const state = loadState();
  const record = state.runtimeSecrets.find((item) => item.ref === input.ref);
  return record ? unsealRuntimeSecret(record) : undefined;
}

export function setRuntimeSecret(input: { ref: string; value: string }): { ref: string; updatedAt: string; rotatedAt?: string } {
  return mutate((state) => {
...
export function registerModelProxyGrant(input: ModelProxyGrantRecord): ModelProxyGrantRecord {
  return mutate((state) => {
...
export function consumeModelProxyGrant(input: { delegationId: string; runId: string; agentConnectionId: string; provider: string; model: string; requestClass: string; nowIso?: string }): { grant: ModelProxyGrantRecord; credential: AgentCredentialRecord } {
  return mutate((state) => {
...
export function revokeModelProxyGrant(input: { delegationId: string; reason?: string }): ModelProxyGrantRecord | undefined {
  return mutate((state) => {
    const grant = state.modelProxyGrants.find((item) => item.delegationId === input.delegationId);
...
export function recordAgentConnectionSmoke(input: { ownerId: string; connectionId: string; ok: boolean; message: string; failureCode?: string; redacted?: boolean }): { agentHome: AgentHome; connection: AgentConnection } {
  return mutate((state) => {
    const connection = state.agentConnections.find((item) => item.id === input.connectionId && item.ownerId === input.ownerId);
    if (!connection) throw new Error("Agent connection not found.");
    const home = state.agentHomes.find((item) => item.id === connection.agentHomeId);
...
export function getAgentHomeConnection(input: { ownerId: string; connectionId: string }): AgentConnection | undefined {
  const state = loadState();
  const connection = state.agentConnections.find((item) => item.id === input.connectionId && item.ownerId === input.ownerId);
  return connection ? withCredentialState(state, clone(connection)) : undefined;
}

export function acquireCodexLoginLease(input: { ownerId: string; leaseId: string; expiresAt: string; nowIso?: string }): boolean {
  return mutate((state) => {
...
export function releaseCodexLoginLease(input: { ownerId: string; leaseId: string }): boolean {
  return mutate((state) => {
...
export function beginClaudeCodeLoginAttempt(input: { ownerId: string; attemptId: string; expiresAt: string; nowIso?: string }): boolean {
  return mutate((state) => {
    const nowMs = Date.parse(input.nowIso || now());
...
export function submitClaudeCodeLoginCode(input: { ownerId: string; attemptId: string; code: string; nowIso?: string }): boolean {
  return mutate((state) => {
    const nowMs = Date.parse(input.nowIso || now());
...
export function takeClaudeCodeLoginCode(input: { ownerId: string; attemptId: string; nowIso?: string }): string | undefined {
  const nowMs = Date.parse(input.nowIso || now());
  const available = loadState().claudeCodeLoginAttempts.some((item) => item.ownerId === input.ownerId && item.attemptId === input.attemptId && Date.parse(item.expiresAt) > nowMs && item.codeSecret);
...
export function releaseClaudeCodeLoginAttempt(input: { ownerId: string; attemptId: string }): boolean {
  return mutate((state) => {
...
export function reserveAgentRun(input: {
  agentHomeId: string;
  connectionId: string;
  challengeId: string;
...
}): { run: AgentRun; job?: Job; reused: boolean } {
  return mutate((state) => {
...
export function getAgentRun(id: string): AgentRun | undefined {
  const run = loadState().agentRuns.find((item) => item.id === id);
  return run ? clone(run) : undefined;
}

export function listAgentRuns(filters: { ownerId?: string; challengeId?: string; connectionId?: string; statuses?: AgentRun["status"][]; since?: string } = {}): AgentRun[] {
  const sinceMs = filters.since ? Date.parse(filters.since) : undefined;
  return loadState().agentRuns
...
export function findAgentRunByIdempotencyKey(input: { challengeId: string; contributorId: string; idempotencyKey: string }): AgentRun | undefined {
  const run = loadState().agentRuns.find((item) => item.challengeId === input.challengeId && item.contributorId === input.contributorId && item.idempotencyKey === input.idempotencyKey);
  return run ? clone(run) : undefined;
}

export function updateAgentRun(input: { id: string; status?: AgentRun["status"]; contributionId?: string; receiptSummary?: AgentRun["receiptSummary"]; failure?: AgentRun["failure"]; jobId?: string }): AgentRun {
  return mutate((state) => {
...
export function updateJob(input: { id: string; status: Job["status"]; latencyMs?: number; costCents?: number; error?: string }): Job {
  return mutate((state) => {
...
export function ensureSeedData() {
  ...
  for (const seed of launchSeedChallenges) {
    const challenge = getChallenge(seed.id) || createChallenge({ id: seed.id, visibility: "public", reward: seed.reward, brief: canonicalChallengePublicationAcknowledgementBrief(seed.brief) });
    if (!seed.withDemoContribution) continue;
    if (listContributions(challenge.id).length === 0) {
    ...
    if (!getLatestSynthesis(challenge.id)) synthesizeChallenge(challenge.id);
  }
}
